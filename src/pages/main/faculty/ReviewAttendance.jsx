...
const ReviewAttendance = (props) => {
    ...
    useEffect(() => {
        ...
        const dep = `studentDepartment=${facultyDepartment}`
        const admiYear = `studentAdmissionYear=${selectedYear}`
        const semester = `studentCourse.semester=${query.semester}`
        const sGroup = `studentCourse.studentGroup=${studentGroup}`
        const sSection = `studentCourse.studentSection=${studentSection}`
        const miniurl = `${dep}&${admiYear}&${degree}&${semester}&${sGroup}&${sSection}`
        if (isDateSelected) {
            const degree = `degree=${query.degree}`
            const dep = `department=${facultyDepartment}`
            const admiYear = `studentAdmissionYear=${selectedYear}`
            const semester = `semester=${query.semester}`
            const sGroup = `studentGroup=${studentGroup}`
            const sSection = `studentSection=${studentSection}`
            const Date = `date=${date}`
            const miniurl2 = `${dep}&${admiYear}&${degree}&${semester}&${sGroup}&${sSection}&${Date}`
    ...
    }, [BASEURL, date, isDateSelected, query.degree, query.department, query.semester, selectedYear, studentGroup, studentSection])
        ...
        <div className='flex flex-col w-full items-center bg-gradient-to-r from-blue-500 to-purple-500'>
            ...
            <div className="container  mx-auto w-11/12 p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    ...
                    {isDateSelected && allStudents[0]?.studentAttendances?.map(student => (
                        <div
                            // key={student.studentEmail}
                            className="bg-white p-4 rounded-md shadow-md cursor-pointer"
                            value={student.studentEmail}
